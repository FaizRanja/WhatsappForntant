...
export default function ShowSendData() {
  ...
  const fetchMessages = async (params = {}) => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:4000/api/v1/qr-scans/user/message', {
        params,
        headers: {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const response = await axios.delete(`http://localhost:4000/api/v1/qr-scans/user/delete/sentmessage/${scanToDelete}`);
...
<AlertSendMessage  alert={alertOpen} close={handleAlertClose}  severity={alertSeverity}  message={alertMessage} />
